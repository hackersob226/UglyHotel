...
public class AvailableRoomsPanel extends JPanel {
    ...
    public void getData(Connection con, String dbName, int id, java.sql.Date startDay, java.sql.Date endDay) throws SQLException {
        Statement stmt = null;
        String query = "SELECT * FROM RESERVATIONHASROOM INNER JOIN ROOM ON ( ROOM.RoomNum = RESERVATIONHASROOM.RoomNum AND ROOM.Location = RESERVATIONHASROOM.Location ) INNER JOIN RESERVATION ON RESERVATION.ReservationID = RESERVATIONHASROOM.ReservationID INNER JOIN USER ON USER.Username = RESERVATION.Username WHERE RESERVATIONHASROOM.ReservationID = \"" + id+ "\" AND NOT EXISTS ( SELECT RESERVATIONHASROOM.RoomNum FROM RESERVATIONHASROOM INNER JOIN RESERVATION ON RESERVATIONHASROOM.ReservationID = RESERVATION.ReservationID INNER JOIN ROOM ON ( ROOM.RoomNum = RESERVATIONHASROOM.RoomNum AND ROOM.Location = RESERVATIONHASROOM.Location ) WHERE IF(RESERVATION.ReservationID != \"" + id+ "\", ROOM.RoomNum = RESERVATIONHASROOM.RoomNum, ROOM.RoomNum != RESERVATIONHASROOM.RoomNum) AND RESERVATION.StartDate <= \"" + endDay+ "\" AND RESERVATION.EndDate >= \"" + startDay + "\")";
        try {
